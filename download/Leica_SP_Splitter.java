...
changed to split a Leica stack into its color channels by Joachim Walter Sept 2001

 */
public class Leica_SP_Splitter  implements PlugIn {
	...
	void openStacks(FileInfo[] fi) throws IOException {
		...
		for (int j=0; j<nr_channels; j++){
			...
			if (stacks[j]!=null){
				...
				String savename;
				if (name.regionMatches(true,name.length()-4,".tif",0,4)){
					savename=name.substring(0,name.length()-4)+"-"+(j+1)+".tif";
				}
				else if (name.regionMatches(true,name.length()-5,".tiff",0,5)){
					savename=name.substring(0,name.length()-5)+"-"+(j+1)+".tif";
				}
				else {
					savename=name+"-"+(j+1)+".tif";
				}
				
				ImagePlus imp = new ImagePlus("", stacks[j]);
				new FileSaver(imp).saveAsTiffStack(dir+savename);
			}
		}
	}
}
